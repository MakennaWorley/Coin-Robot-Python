...
class coin_robot:
    ...
    def solve(self):
        graph = [[0] * self.column for _ in range(self.row)]
        path = [[[]] * self.column for _ in range(self.row)]

        for i in range(self.row):
            for j in range(self.column):
                value_of_square = max(graph[i-1][j], graph[i][j-1])
                graph[i][j] = value_of_square + self.map[i][j]
